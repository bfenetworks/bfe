...
func CallCustomCodec(codec grpc.Codec) CallOption {
...
func (p *parser) recvMsg(maxReceiveMessageSize int) (pf payloadFormat, msg []byte, err error) {
	...
	if int64(length) > math.MaxInt64 {
		return 0, nil, status.Errorf(codes.ResourceExhausted, "grpc: received message larger than max length allowed on current machine (%d vs. %d)", length, math.MaxInt64)
	}
