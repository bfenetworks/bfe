...
//lint:file-ignore U1000 Ignore type field is unused
...
func (fr *Framer) maxHeaderUriSize() uint32 {
	if fr.MaxHeaderUriSize == 0 {
		return http.DefaultMaxHeaderUriBytes
	}
	return fr.MaxHeaderUriSize
}
...
func parseWindowUpdateFrame(fh FrameHeader, p []byte) (Frame, error) {
	...
	if inc == 0 {
		...
		return nil, StreamError{fh.StreamID, ErrCodeProtocol, errMsg}
	}
...
func (mh *MetaHeadersFrame) RegularValue(regular string) string {
...
func (fr *Framer) checkHeaderFieldLimit(f *HeadersFrame, hf *hpack.HeaderField) error {
	if hf.Name == ":path" {
		if len(hf.Value) > int(fr.maxHeaderUriSize()) {
...
func (fr *Framer) readMetaFrame(f *HeadersFrame) (*MetaHeadersFrame, error) {
	if fr.AllowIllegalReads {
	...
	hdec.SetEmitFunc(func(hf hpack.HeaderField) error {
		headerSize += hf.FieldSize()
		if err := fr.checkHeaderFieldLimit(f, &hf); err != nil {
			invalid = err
			return err
		}
	...
	defer hdec.SetEmitFunc(func(hf hpack.HeaderField) error { return nil })
	...
	for {
		...
		if _, err = hdec.Write(frag); err != nil {
			...
			switch err := err.(type) {
			case maxHeaderUriSizeError:
				return nil, err
			case maxHeaderListSizeError:
				return nil, err
			}
