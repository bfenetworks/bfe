...
func TestReadFrameOrder(t *testing.T) {
	...
	for i, tt := range tests {
		...
		if !ok && !checkEqual(err, ConnectionError{Code: ErrCodeProtocol}) {
...
func checkEqual(want, got interface{}) bool {
	// for StreamError, just check StreamID and Code
	wse, ok1 := want.(StreamError)
	gse, ok2 := got.(StreamError)
	if ok1 && ok2 {
		return wse.StreamID == gse.StreamID && wse.Code == gse.Code
	}

	// for ConnectionError, just check Code
	wce, ok1 := want.(ConnectionError)
	gce, ok2 := got.(ConnectionError)
	if ok1 && ok2 {
		return wce.Code == gce.Code
	}
