...
func TestNewRSAOEAP256(t *testing.T) {
	...
	token, _ := ioutil.ReadFile(fmt.Sprintf("%s/test_jwe_RSA-OAEP-256_A128GCM.txt", path))
	secret, _ := ioutil.ReadFile(fmt.Sprintf("%s/secret_test_jwe_RSA-OAEP-256_A128GCM.key", path))
