...
const (
	...
	DefaultConcurrency = 50                      //default concurrency for async check job
)
...
type ConfModWaf struct {
	...
	Basic struct {
		...
		Concurrency     int    //concurrency for async check job
	}
	//waf log config
	Log struct {
...
func (cfg *ConfModWaf) Check(confRoot string) error {
	...
	if cfg.Basic.Concurrency <= 0 {
		log.Logger.Warn("ConfModWaf.Concurrency not set, use default value")
		cfg.Basic.Concurrency = DefaultConcurrency
	}
