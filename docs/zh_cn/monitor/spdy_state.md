...
| SPDY_ERR_STREAM_ALREADY_CLOSED | 流已关闭的错误数        |
| SPDY_ERR_STREAM_CANCEL         | 流被取消的错误数        |
| SPDY_ERR_NEW_FRAMER            | 新建Framer失败的错误数  |
| SPDY_PANIC_CONN                | 连接处理协程panic的次数 |
| SPDY_PANIC_STREAM              | 流处理协程panic的次数   |
| SPDY_REQ_HEADER_COMPRESS_SIZE  | 压缩的请求头总字节数    |
| SPDY_REQ_HEADER_ORIGINAL_SIZE  | 原始的请求头总字节数    |
...
| SPDY_REQ_OVERLOAD              | SPDY请求过载的次数      |
| SPDY_CONN_OVERLOAD             | SPDY连接过载的次数      |
