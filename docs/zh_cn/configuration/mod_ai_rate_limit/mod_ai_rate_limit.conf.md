...
> 说明：本文件中的 `Redis` 配置仅供限流计数使用。AI Key 会话级亲和性（`AIConf.KeyPolicy.SessionAffinity`）的存储后端由 `bfe.conf` 的 `[AIKeyAffinity]` 段独立配置（bfe_server 自持连接）；未配置该段时亲和能力不启用（fail-open），与本模块的加载及 Redis 配置无关。
