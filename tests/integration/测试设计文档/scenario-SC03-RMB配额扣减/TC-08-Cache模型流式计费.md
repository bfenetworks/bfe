...
验证当模型价格表中配置了 cache 相关单价时，BFE 对流式（SSE）响应按 cache 拆分公式计费。cache usage 通常只在 SSE 流的最终 chunk 中出现。
...
3. mock 后端 `cluster_rmb` 已启动，返回 `Content-Type: text/event-stream` 的 SSE 流，最终 chunk 含 cache usage：
   ```
