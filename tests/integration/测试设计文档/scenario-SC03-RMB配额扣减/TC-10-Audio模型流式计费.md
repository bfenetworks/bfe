...
验证当模型价格表中配置了音频相关单价时，BFE 对 SSE 流式响应按音频 input/output 拆分公式计费。
...
3. mock 后端 `cluster_rmb` 已启动，返回 SSE 流，最终 chunk 包含 audio usage：
   ```text
...
| Body | `{"model":"gpt-audio-1.5","stream":true}` |
